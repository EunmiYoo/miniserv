...
int main(int argc, char **argv) {
    ...
    if (listen(sockfd, 1024) != 0)
        fatal();
    FD_ZERO(&_fds);//fd_set의 모든 비트를 0으로 초기화 
    FD_SET(sockfd, &_fds);//sockfD로 전달된  fd_set의 인덱스를 1로 설정 .sockfd를 관리대상으로 지정 
    fd_max = sockfd; //최대 파일 디스크립터 값 
//select: 어느 소켓의 fd에 read, write, exception 이 발생했는지 확인하는 함수,변화가 발생한 소켓의 디스크립터만 1로 설정 
    for (;;)
    {
